...
public class Mp3Lame extends Activity {
	...
	private void processRecord()
	{
		...
		final int minBufferSize =
				AudioRecord.getMinBufferSize (mSampleRate, AudioFormat.CHANNEL_IN_STEREO, AudioFormat.ENCODING_PCM_16BIT);
		if (minBufferSize < 0) {
			mHandler.sendEmptyMessage (MsgType.ErrorGetMinBuffersize.getValue());
			return;
		}
//		AudioRecord audioRecord =
//				new AudioRecord (MediaRecorder.AudioSource.CAMCORDER, mSampleRate,
//								 AudioFormat.CHANNEL_IN_STEREO,
//								 AudioFormat.ENCODING_PCM_16BIT, minBufferSize * 2);
		
		AudioRecord audioRecord = new AudioRecord (5,44100,1,2,minBufferSize * 2);

	
		// PCM buffer size (5sec)
		short[] buffer = new short [mSampleRate * 16 * 2 * 5]; // SampleRate[Hz] * 16bit * Mono * sec
		...
		LameWrapper.init (mSampleRate, 1, mSampleRate, 128);
//		LameWrapper.Init(1,44100,128,0,3);
